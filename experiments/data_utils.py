...
ds_properties = {
    ...
    "alpaca-cleaned": {"path": "yahma/alpaca-cleaned"},
    "alpaca": {"path": "tatsu-lab/alpaca", "cols_to_remove": ['input', 'output', 'instruction']},
}


def format_into_chat(example: dict, eos_token: str):
    ...
    return template.render(messages=messages, eos_token=eos_token)


def format_dataset_from_path(path: str, eos_token: str) -> (list, list):
    ...
    system_msg = "You are a helpful AI assistant. Keep your answers short and kind. Always give answers that are safe, inclusive, and helpful. Never give out information which could lead to harm."
    ...
    for example in ds["train"]:
        for ex in example.values():
            ...
            train_samples.append(template.render(messages=messages, eos_token=eos_token))
    ...
    for example in ds["test"]:
        for ex in example.values():
            ...
            for turn in ex:
                messages.append(turn)
            test_samples.append(template.render(messages=messages, eos_token=eos_token))
...
def prepare_test_dataloader_like_baseline(
    ...
    """
    Get a DataLoader from a test dataset. This dataloader should be
    used when comparing WikiText2 perplexities for std models with other papers
